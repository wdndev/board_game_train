...
class MobileCGTBlockv3(nn.Module):
    def __init__(self, 
                 inp, 
                 embed_dim,         # 词嵌入维度
        ...
        super(MobileCGTBlockv3, self).__init__()
...
class MobileCGTv3(nn.Module):
    def __init__(self, in_c, 
        ...
        """
            mobile computer game transformer with self attention
            - ffn_multiplier : transformer ffn 隐藏层倍数
            - last_layer_exp_factor : 最后输出 1*1 卷积倍数
        """
        super().__init__()

        self.model_name = "mobile_cgt_v3"
        ...
        self.layer_2 = nn.Sequential(
            ...
            MobileCGTBlockv3(channels[2], embed_dim[0], ffn_multiplier, heads=4, dim_head=8, attn_blocks=2)
        )

        self.layer_3 = nn.Sequential(
            InvertedResidual(channels[2], channels[3], stride=1, expand_ratio=2),
            MobileCGTBlockv3(channels[3], embed_dim[1], ffn_multiplier, heads=4, dim_head=8, attn_blocks=4)
        )

        self.layer_4 = nn.Sequential(
            InvertedResidual(channels[3], channels[4], stride=1, expand_ratio=2),
            MobileCGTBlockv3(channels[4], embed_dim[2], ffn_multiplier, heads=4, dim_head=8, attn_blocks=3)
        )
...
def mobile_cgt_v3(in_chans=4, board_size=19):
    """ Total of parameters: 1317156
    """
    model = MobileCGTv3(in_c=in_chans)
    model.model_name = "mobile_cgt_v3"
    return model

def mobile_cgt_v3_1_6m(in_chans=4, board_size=19):
    """ Total of parameters: 1317156
    """
    model = MobileCGTv3(in_c=in_chans, channels = [16, 32, 64, 64, 96])
    model.model_name = "mobile_cgt_v3_1_6m"
    return model
...
if __name__ == "__main__":
    ...
    # mcb = MobileCGTBlockv3(inp=30, embed_dim=64, ffn_multiplier=2, heads=4, dim_head=8, attn_blocks=2)

    # x = mcb(tensor)
    # print(x.shape)

    mcgt = mobile_cgt_v3_1_6m(in_chans=4)
