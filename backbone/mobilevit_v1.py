""" https://github.com/jaiwei98/mobile-vit-pytorch/tree/main/mobile_vit
"""
import math
import torch
...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim, ffn_hidden_dim, heads=8, dim_head=8, dropout=0, attn_dropout=0):
        super().__init__()
        ...
        self.pre_norm_ffn = nn.Sequential(
            nn.LayerNorm(embed_dim, eps=1e-5, elementwise_affine=True),
            nn.Linear(embed_dim, ffn_hidden_dim, bias=True),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Linear(ffn_hidden_dim, embed_dim, bias=True),
            nn.Dropout(dropout)
        )
...
class MobileViT(nn.Module):
    def __init__(self, in_c, patch_size=(2, 2)):  
        super().__init__()
        # check image size
        # bh, bw = board_size, board_size
        # self.ph, self.pw = patch_size
        # # assert bh % self.ph == 0 and bw % self.pw == 0 
        ...
        self.layer_1 = nn.Sequential(
            ...
            InvertedResidual(channels[1], channels[1], stride=1, expand_ratio=mv2_exp_mult),
        )
        self.layer_2 = nn.Sequential(
            InvertedResidual(channels[1], channels[2], stride=1, expand_ratio=mv2_exp_mult),
            MobileViTBlock(channels[2], embed_dim[0], ffn_multiplier, heads=4, dim_head=8, attn_blocks=2, patch_size=patch_size)
        )

        self.layer_3 = nn.Sequential(
            InvertedResidual(channels[2], channels[3], stride=1, expand_ratio=mv2_exp_mult),
            MobileViTBlock(channels[3], embed_dim[1], ffn_multiplier, heads=4, dim_head=8, attn_blocks=4, patch_size=patch_size)
        )

        self.layer_4 = nn.Sequential(
            InvertedResidual(channels[3], channels[4], stride=1, expand_ratio=mv2_exp_mult),
            MobileViTBlock(channels[4], embed_dim[2], ffn_multiplier, heads=4, dim_head=8, attn_blocks=3, patch_size=patch_size)
        )
...
def mobile_vit(in_chans=4, board_size=19):
    """ Total of parameters: 1677780
    """
    model = MobileViT(in_c=in_chans)
    model.model_name = "mobile_vit"
    return model
...
if __name__ == "__main__":
    ...
    # mvb = MobileViTBlock(inp=64, embed_dim=64, ffn_multiplier=2, heads=4, dim_head=8, attn_blocks=2, patch_size=(2,2))

    # x = mvb(x)
    # print(x.shape)

    mvit = mobile_vit(in_chans=2)

    p_hat, v_hat = mvit(tensor)

    print("p_hat: ", p_hat.shape)
    print("v_hat: ", v_hat.shape)

    # print_model_parameters(mvit)
